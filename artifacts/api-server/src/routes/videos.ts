...
async function getVideoWithArtist(id: number, userId?: number) {
  const [row] = await db
    .select({
      ...
      platformAssignedTag: videosTable.platformAssignedTag,
      tagSource: videosTable.tagSource,
      aiDetectionReasons: videosTable.aiDetectionReasons,
      aiOverrideReason: videosTable.aiOverrideReason,
      appealStatus: videosTable.appealStatus,
    })
    .from(videosTable)
...
router.patch("/videos/:id", requireAuth, async (req: AuthRequest, res): Promise<void> => {
  ...
  if (parsed.data.status === "published" && isFutureRelease(existing.releaseDate)) {
    res.status(409).json({ error: `This video is scheduled to release on ${existing.releaseDate} and cannot be published early.` });
    return;
  }
