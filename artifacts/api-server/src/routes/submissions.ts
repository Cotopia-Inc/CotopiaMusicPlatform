...
router.post("/submissions", requireAuth, requireVerifiedEmail, async (req: AuthRequest, res): Promise<void> => {
  ...
  // Extract creationMethod from body (not yet in generated Zod schema — read separately)
  const rawCreationMethod = (req.body as Record<string, unknown>).creationMethod;
  const ALLOWED_METHODS = ["unclassified","human_created","ai_assisted","hybrid_human_ai","fully_ai_generated"];
  const creationMethod = (ALLOWED_METHODS.includes(rawCreationMethod as string) ? rawCreationMethod : "unclassified") as string;
  ...
  if (d.type === "song") {
    const [song] = await db.insert(songsTable).values({
      ...
      creationMethod,
      creatorSelectedTag: creationMethod === "unclassified" ? null : creationMethod,
      effectiveDisplayTag: creationMethod,
      tagSource: creationMethod === "unclassified" ? null : "creator",
    }).returning();
    contentId = song.id;
  } else {
  ...
  if (creationMethod === "fully_ai_generated" && autoRejectFullyAi) {
    ...
    const [submission] = await db.insert(submissionsTable).values({
      ...
      submitterNotes,
      creationMethod,
      aiReviewStatus: "auto_rejected",
    }).returning();
    await db.insert(adminAuditLogsTable).values({
      adminUserId: req.user!.userId,
      action: "fully_ai_auto_rejected",
    ...
    res.status(422).json({
      ...
      code: "FULLY_AI_REJECTED",
      policy: "Everyday Radio welcomes human-created, AI-assisted, and hybrid creative work. Content that is fully generated by artificial intelligence without meaningful human creative authorship is not accepted at this time.",
      appealLink: "/trust/appeals",
    });
    return;
  }
...
router.post("/submissions/bulk", requireAuth, requireVerifiedEmail, async (req: AuthRequest, res): Promise<void> => {
  ...
  // Extract creationMethod from body (read separately since not yet in generated Zod schema)
  const rawCreationMethod = (req.body as Record<string, unknown>).creationMethod;
  const ALLOWED_METHODS = ["unclassified","human_created","ai_assisted","hybrid_human_ai","fully_ai_generated"];
  const creationMethod = (ALLOWED_METHODS.includes(rawCreationMethod as string) ? rawCreationMethod : "unclassified") as string;
