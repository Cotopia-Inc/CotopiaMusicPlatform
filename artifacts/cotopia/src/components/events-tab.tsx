...
import { CalendarDays, List, Pencil, Plus, Trash2, MapPin, Link as LinkIcon, Sparkles, Loader2, Upload, ImageIcon, X, CalendarPlus } from "lucide-react";
...
function icsDate(dateStr: string, allDayEnd = false): string {
  const d = parseISO(dateStr);
  if (allDayEnd) d.setDate(d.getDate() + 1);
  return format(d, "yyyyMMdd");
}

function buildIcsContent(event: Event): string {
  const escape = (s: string) => s.replace(/[\\,;]/g, (m) => `\\${m}`).replace(/\n/g, "\\n");
  const dtStart = icsDate(event.eventDate);
  const dtEnd = icsDate(event.endDate ?? event.eventDate, true);
  const lines = [
    "BEGIN:VCALENDAR",
    "VERSION:2.0",
    "PRODID:-//Cotopia//Events//EN",
    "CALSCALE:GREGORIAN",
    "BEGIN:VEVENT",
    `UID:cotopia-event-${event.id}@cotopia`,
    ...
    event.link ? `URL:${escape(event.link)}` : "",
    "END:VEVENT",
    "END:VCALENDAR",
  ].filter(Boolean);
  return lines.join("\r\n");
...
function googleCalendarUrl(event: Event): string {
  const start = icsDate(event.eventDate);
  const end = icsDate(event.endDate ?? event.eventDate, true);
  ...
  return (
    <DropdownMenu>
      <DropdownMenuTrigger asChild>
        <Button variant="ghost" size="icon" className="h-7 w-7" title="Pin to calendar">
          <CalendarPlus className="w-3.5 h-3.5" />
        </Button>
      </DropdownMenuTrigger>
      <DropdownMenuContent align="end">
        <DropdownMenuItem onClick={() => window.open(googleCalendarUrl(event), "_blank", "noopener,noreferrer")}>
          Google Calendar
        </DropdownMenuItem>
