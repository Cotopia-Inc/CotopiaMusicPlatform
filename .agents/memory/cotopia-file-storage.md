...
The upload endpoint buffers the whole request body in server memory via `express.raw({limit:...})` before writing to the backend. This is a known tradeoff (not yet fixed) — full streaming was scoped out because R2's REST PUT can't be safely converted to a streamed body without real R2 credentials to test against (Render prod uses R2; this dev environment only has GCS configured, so R2 changes are unverifiable here). If revisiting, only stream the GCS/local paths (both testable in this dev env) and leave R2 buffered, or test R2 streaming against real credentials first.

**No product-level file size cap by design (as of 2026-07-04):** the `express.raw` limit is set to `10gb` as a pure technical safety ceiling against OOM from a garbage/malicious request — not a real-world restriction. The user explicitly asked for "no size limit" since creators upload large "one take" videos. `artifacts/cotopia/src/pages/submit.tsx`'s client-side `MAX_FILE_SIZE_BYTES` mirrors this same 10GB ceiling (not a business rule) and shows a friendly toast if ever hit. `app.ts` has a body-parser error handler that returns a clean JSON 413 instead of Express's default HTML error page. If this ceiling is ever hit in practice, the fix is raising the ceiling further or revisiting streaming — not reintroducing a low business cap.
